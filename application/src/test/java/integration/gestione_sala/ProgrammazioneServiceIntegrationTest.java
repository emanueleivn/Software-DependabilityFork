package integration.gestione_sala;

import it.unisa.application.database_connection.DataSourceSingleton;
import it.unisa.application.model.dao.ProiezioneDAO;
import it.unisa.application.sottosistemi.gestione_sala.service.ProgrammazioneService;
import org.junit.jupiter.api.BeforeAll;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import unit.test_DAO.DatabaseSetupForTest;

import java.sql.Connection;
import java.sql.Statement;
import java.time.LocalDate;
import java.util.List;

import static org.junit.jupiter.api.Assertions.*;

class ProgrammazioneServiceIntegrationTest {
    private ProgrammazioneService programmazioneService;
    private LocalDate futureDate;

    @BeforeAll
    static void globalSetup() {
        DatabaseSetupForTest.configureH2DataSource();
    }

    private void cleanAndSeed() {
        try (Connection conn = DataSourceSingleton.getInstance().getConnection();
             Statement stmt = conn.createStatement()) {

            // cleanup inline (niente proiezione_slot)
            stmt.execute("SET REFERENTIAL_INTEGRITY FALSE");
            stmt.execute("DELETE FROM posto_proiezione");
            stmt.execute("DELETE FROM prenotazione");
            stmt.execute("DELETE FROM proiezione");
            stmt.execute("DELETE FROM slot");
            stmt.execute("DELETE FROM sala");
            stmt.execute("DELETE FROM film");
            stmt.execute("DELETE FROM utente");
            stmt.execute("DELETE FROM sede");
            stmt.execute("SET REFERENTIAL_INTEGRITY TRUE");

            // seed minimo per i test
            stmt.execute("INSERT INTO sede (id, nome, via, citta, cap) VALUES (1, 'Moviplex', 'Via Roma', 'Napoli', '80100');");
            stmt.execute("INSERT INTO film (id, titolo, durata, genere, classificazione, descrizione, is_proiettato) " +
                    "VALUES (1, 'Film Test', 120, 'Azione', 'T', 'Descrizione di test', TRUE);");
            stmt.execute("INSERT INTO sala (id, numero, capienza, id_sede) VALUES (1, 1, 100, 1);");
            stmt.execute("INSERT INTO slot (id, ora_inizio) VALUES (1, '10:00:00');");
            stmt.execute("INSERT INTO slot (id, ora_inizio) VALUES (2, '12:00:00');");
        } catch (Exception e) {
            throw new RuntimeException("Errore nel reset/seed del database di test", e);
        }
    }

    @BeforeEach
    void setUp() {
        futureDate = LocalDate.now().plusDays(7);
        cleanAndSeed();
        programmazioneService = new ProgrammazioneService(new ProiezioneDAO());
    }

    @Test
    void testFilmNonSelezionato() {
        boolean result = programmazioneService.aggiungiProiezione(0, 1, List.of(1, 2), futureDate);
        assertFalse(result);
    }

    @Test
    void testDataNonInserita() {
        boolean result = programmazioneService.aggiungiProiezione(1, 1, List.of(1, 2), null);
        assertFalse(result);
    }

    @Test
    void testDataPassata() {
        boolean result = programmazioneService.aggiungiProiezione(1, 1, List.of(1, 2), LocalDate.now().minusDays(1));
        assertFalse(result);
    }

    @Test
    void testSlotNonDisponibili() {
        boolean result = programmazioneService.aggiungiProiezione(1, 1, List.of(10, 20), futureDate);
        assertFalse(result);
    }

    @Test
    void testProiezioneAggiuntaConSuccesso() {
        boolean result = programmazioneService.aggiungiProiezione(1, 1, List.of(1, 2), futureDate);
        assertTrue(result, "Il test dovrebbe passare perché tutti i parametri sono validi.");
    }
}
